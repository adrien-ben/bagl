...
public class Spritebatch {
	...
	public void draw(Texture texture, Vector2 position, float width, float height) {
		...
		this.vertices[offset] = new VertexPositionTexture(new Vector3(position.getX(), position.getY(), 0), 
				new Vector2(xTexelOffset, yTexelOffset));
		this.vertices[offset + 1] = new VertexPositionTexture(new Vector3(position.getX() + width, position.getY(), 0), 
				new Vector2(1 - xTexelOffset, yTexelOffset));
		this.vertices[offset + 2] = new VertexPositionTexture(new Vector3(position.getX(), position.getY() + height, 0), 
				new Vector2(xTexelOffset, 1 - yTexelOffset));
		this.vertices[offset + 3] = new VertexPositionTexture(new Vector3(position.getX() + width, position.getY() + height, 0), 
				new Vector2(1 - xTexelOffset, 1 - yTexelOffset));
